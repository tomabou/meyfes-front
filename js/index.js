		...
		switch (msg.$) {
			...
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{routeRatio: 1 - model.routeRatio, showRoute: !model.showRoute}),
					elm$core$Platform$Cmd$none);
		}
	});
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'GraphCreated':
				var graph = msg.a;
				var oldGraph = model.gridGraph;
				var newGraph = _Utils_update(
					oldGraph,
					{edgeC: graph.edgeC, edgeR: graph.edgeR, vertex: graph.vertex});
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{converteState: author$project$Main$Done, gridGraph: newGraph}),
					elm$core$Platform$Cmd$none);
			case 'FailedCreateGraph':
				var err = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							converteState: author$project$Main$Error(err)
						}),
					elm$core$Platform$Cmd$none);
			default:
				var graphMsg = msg.a;
				var _n1 = A2(author$project$Graph$update, graphMsg, model.gridGraph);
				var graph = _n1.a;
				var cmd = _n1.b;
				return _Utils_Tuple2(
					_Utils_update(
...
var author$project$Graph$ShowRoute = {$: 'ShowRoute'};
...
var author$project$Main$renderSquare = A2(
	joakin$elm_canvas$Canvas$shapes,
	_List_fromArray(
		[
			joakin$elm_canvas$Canvas$fill(
			A4(avh4$elm_color$Color$rgba, 0.5, 0.5, 0.5, 0.5))
		]),
	_List_fromArray(
		[
			A3(
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								joakin$elm_canvas$Canvas$fill(avh4$elm_color$Color$green)
							]),
						_List_fromArray(
							[
								A3(
